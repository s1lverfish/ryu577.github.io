...
these states. In the Markov assumption, the probability of the state we will go to next only depends on the state we are in now. So, we can write these probabilities 
...
Experiments show that for this simple Markov chain, the optimal threshold doesn't change. However, in more complex markov chains with multiple thresholds, 
we might want to optimize this objective function.
		
### 3.2. Non-parametric approach
Let's say we only wanted to evaluate thresholds that were less than the minimum censored value (say our current threshold is 10 minutes and so, all data points are censored
at ten minutes and we're sure we want to decrease it - so evaluate thresholds less than 10 minutes). 
...
We can also directly model hazard rates as a function of some covariates using the piecewise exponential model described in [1] and [2].
...
We can then see how various approaches perform given just some data generated by this process.

### 5.1. Non-parametric approaches when we know the distribution
...
![Complete censoring]({{ site.url }}/Downloads/opt_thresholds/no_censor_non_parmtr.png)

For the Weibull distribution, we get much higher optimal thresholds and for the LogLogistic distribution, much lower ones. This highlights how risky it
can be to use parametric models. If our assumption is incorrect, we can end up way off in terms of the optimal wait threshold.

#### 5.1.2. Complete censoring
Here, we consider the case where we completely censor the data at some censor level. This process describes the scenario where we always wait exactly 10 minutes (say)
for the bus and then start walking. How do our non-parametric approaches do in this scenario? 

#### 5.1.3. Random censoring



### 5.2. Non-standard processes
